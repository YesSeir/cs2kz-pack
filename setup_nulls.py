...
def download_allownulls(cs2_dir: str):
    """Скачивает и устанавливает плагин AllowNulls."""
    print(f"Downloading AllowNulls plugin...")

    # URL API последнего релиза
    api_url = "https://api.github.com/repos/yesseir/AllowNulls/releases/latest"
    # Папка для установки плагинов CounterStrikeSharp
    plugins_path = os.path.join(cs2_dir, "game", "csgo", "addons", "counterstrikesharp", "plugins")
    ...
    try:
        ...
        # 2. Ищем в релизе файл архива (AllowNulls.zip)
        asset_url = None
        asset_name = None
        ...
        if not asset_url:
            raise Exception("No zip archive found in the latest release.")
        ...
        # 4. Распаковываем архив прямо в папку plugins
        print(f"Extracting to {plugins_path}...")
        with zipfile.ZipFile(asset_name, 'r') as zip_ref:
            zip_ref.extractall(plugins_path)

        # 5. Удаляем временный архив
        os.remove(asset_name)
        print(f"AllowNulls plugin installed successfully.")

    except Exception as e:
        print(f"Warning: Failed to download/install AllowNulls plugin: {e}")
...
try:
    download_allownulls(path)
except Exception as e:
    print(f"Warning: Failed to download AllowNulls: {e}")
